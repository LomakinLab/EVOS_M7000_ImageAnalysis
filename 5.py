...
def analyze_and_plot(df_param, param_name, output_dir):
    ...
    if is_normal:
        ref_val = stats_df[stats_df['Condition'] == first_condition]['Value'].mean()
        agg_func = 'mean'
    else:
        ref_val = stats_df[stats_df['Condition'] == first_condition]['Value'].median()
        agg_func = 'median'
    ...
    try:
        ...
        if is_normal:
            plot_type = 'Barplot'
            sns.barplot(x='Condition', y='Value', data=long_df, palette=colors, capsize=0.1, errorbar='se', order=conditions_list, ax=ax)
            print("Plotting: Barplot (Normal Data)")
        else:
            plot_type = 'Boxenplot'
            sns.boxenplot(x='Condition', y='Value', data=long_df, palette=colors, order=conditions_list, showfliers=False, ax=ax)
